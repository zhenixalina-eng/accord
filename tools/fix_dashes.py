...
RULES = [
    ...
    # тире, прижатое к слову с одной стороны: «неандерталы —а может»
    (re.compile(rf'(\s){DASH}(?=[^\s\d])'), r'\1— '),
    (re.compile(rf'(?<=[^\s\d]){DASH}(\s)'), r' —\1'),
    # тире в конце строки. Пробелы ищем без \s: он съедает и перевод строки,
    # из-за чего строка склеивалась со следующей.
    (re.compile(rf'(\S)[^\S\n]+{DASH}[^\S\n]*$', re.M), r'\1 —'),
